insert into sys_dept (id, name, sort, leader, phone, email, status, deleted, parent_id, created_time, updated_time, tenant_id)
values (2048601258595581952, '测试', 0, null, null, null, 1, 0, null, now(), null, 0);
...
insert into sys_role (id, name, status, is_filter_scopes, remark, created_time, updated_time, tenant_id)
values (2048601263515500544, '测试', 1, true, null, now(), null, 0);

insert into sys_role_menu (id, role_id, menu_id, tenant_id)
values
(2048601263578415104, 2048601263515500544, 2049629108245233664, 0),
(2048601263641329664, 2048601263515500544, 2049629108245233665, 0),
(2048601263708438528, 2048601263515500544, 2049629108245233666, 0),
(2048601263775547392, 2048601263515500544, 2049629108253622282, 0);

insert into sys_user (id, uuid, username, nickname, password, salt, email, status, is_superuser, is_staff, is_multi_login, avatar, phone, join_time, last_login_time, last_password_changed_time, dept_id, created_time, updated_time, tenant_id)
values
(2048601263834267648, uuid(), 'admin', '用户88888', '$2b$12$8y2eNucX19VjmZ3tYhBLcOsBwy9w1IjBQE4SSqwMDL5bGQVp2wqS.', unhex('24326224313224387932654E7563583139566A6D5A33745968424C634F'), 'admin@example.com', 1, true, true, true, null, null, now(), now(), now(), 2048601258595581952, now(), null, 0),
(2049946297615646720, uuid(), 'test', '用户66666', '$2b$12$BMiXsNQAgTx7aNc7kVgnwedXGyUxPEHRnJMFbiikbqHgVoT3y14Za', unhex('24326224313224424D6958734E514167547837614E63376B56676E7765'), 'test@example.com', 1, false, false, false, null, null, now(), now(), now(), 2048601258595581952, now(), null, 0);

insert into sys_user_role (id, user_id, role_id, tenant_id)
values
(2048601263838461952, 2048601263834267648, 2048601263515500544, 0),
(2049946493732913152, 2049946297615646720, 2048601263515500544, 0);

insert into sys_data_scope (id, name, status, created_time, updated_time)
values
(2048601263901376512, '本部门数据权限', 1, now(), null),
(2048601263968485376, '部门及以下数据权限', 1, now(), null),
(2048601263968485377, '仅本人数据权限', 1, now(), null),
(2048601263968485378, '全模型本部门数据权限', 1, now(), null),
(2048601263968485379, '排除超级管理员数据权限', 1, now(), null);
...
(2048601264102703104, '部门名称等于测试', 'Dept', 'name', 1, 0, '测试', now(), null),
(2048601264102703105, '父部门 ID 等于测试部门 ID', 'Dept', 'parent_id', 0, 0, '1', now(), null),
...
insert into sys_role_data_scope (id, role_id, data_scope_id, tenant_id)
values
(2048601264169811968, 2048601263515500544, 2048601263901376512, 0),
(2048601264236920832, 2048601263515500544, 2048601263968485376, 0);
